#!/usr/bin/env python3
"""
build.py - articles/ 디렉토리의 HTML 기사를 스캔하여 index.html을 자동 생성합니다.
...
def extract_title(filepath: Path) -> str:
    """HTML 파일에서 <title> 태그 내용을 추출합니다."""
    try:
        content = filepath.read_text(encoding="utf-8")
        match = re.search(r"<title>(.*?)</title>", content, re.IGNORECASE | re.DOTALL)
        if match:
            title = match.group(1).strip()
            # " - IT之家", " | IT之家" 등 소스 표기 제거
...
def extract_excerpt(filepath: Path) -> str:
    """HTML 파일에서 meta description 또는 본문 첫 문단을 추출합니다."""
    try:
        content = filepath.read_text(encoding="utf-8")
        # meta description
        match = re.search(
...
def scan_articles():
    """articles/ 디렉토리를 스캔하여 기사 목록을 반환합니다."""
    articles = []
    ...
    for date_dir in sorted(ARTICLES_DIR.iterdir(), reverse=True):
        if not date_dir.is_dir():
            continue
        date_name = date_dir.name  # e.g. 2026-03-19
...
def generate_article_card(article: dict) -> str:
    ...
    cat_info = CATEGORY_RULES.get(cat, CATEGORY_RULES["camera"])
    tag_class = f"tag-{cat}"
    date_display = article["date"].replace("-", ".")
    excerpt = html.escape(article["excerpt"]) if article["excerpt"] else ""
    title_escaped = html.escape(article["title"])

    return f"""          <a href="{article['path']}" class="article-card" data-category="{cat}">
            <div class="card-category-bar" style="background:{cat_info['color']}"></div>
            <div class="card-body">
              <div class="card-meta">
                <span class="card-tag {tag_class}">{cat_info['label']}</span>
...
def generate_date_group(date_str: str, articles: list) -> str:
    """날짜 그룹 HTML을 생성합니다."""
    date_korean = format_date_korean(date_str)
    cards = "\n".join(generate_article_card(a) for a in articles)
    return f"""      <div class="date-group fade-in">
        <div class="date-header">
          <h2>{date_korean}</h2>
          <span class="date-badge">{len(articles)}건</span>
        </div>
...
def generate_html(articles: list) -> str:
    ...
    for cat_id, cat_info in CATEGORY_RULES.items():
        if cat_counts.get(cat_id, 0) > 0:
            cat_buttons.append(
                f'        <button class="cat-btn" onclick="filterCategory(\'{cat_id}\', this)">'
                f'<span class="cat-dot" style="background:{cat_info["color"]}"></span>'
                f'{cat_info["label"]}</button>'
            )
    cat_buttons_html = "\n".join(cat_buttons)
    ...
    for cat_id, cat_info in CATEGORY_RULES.items():
        ...
        if count > 0:
            cat_list_items.append(
                f'          <li><span class="cat-name">'
                f'<span class="cat-dot" style="background:{cat_info["color"]}"></span>'
                f'{cat_info["label"]}</span>'
                f'<span class="cat-count">{count}</span></li>'
            )
    cat_list_html = "\n".join(cat_list_items)
    ...
    for a in articles:
        try:
            ...
            month_key = f"{d.year}년 {d.month}월"
            by_month[month_key] += 1
        except ValueError:
            pass
    archive_items = "\n".join(
        f'          <li><a href="#"><span>{m}</span><span>{c}건</span></a></li>'
...
<head>
  ...
  <title>AI시테이 블로그 | IT 뉴스 & 테크 리뷰</title>
  <meta name="description" content="IT之家 최신 기사를 한국어로 번역하여 제공하는 테크 블로그">
  ...
  <style>
    ...
    * {{ margin: 0; padding: 0; box-sizing: border-box; }}
    ...
    .tag-camera {{
      background: color-mix(in srgb, var(--cat-camera), transparent 88%);
      color: var(--cat-camera);
    }}

    .tag-phone {{
      background: color-mix(in srgb, var(--cat-phone), transparent 88%);
      color: var(--cat-phone);
    }}

    .tag-ai {{
      background: color-mix(in srgb, var(--cat-ai), transparent 88%);
      color: var(--cat-ai);
    }}
  ...
  <header class="header">
    <div class="header-inner">
      ...
      <nav class="nav-desktop">
        ...
        <a href="#" class="nav-link">IT뉴스</a>
        <a href="#" class="nav-link">카메라</a>
        <a href="#" class="nav-link">스마트폰</a>
        <a href="#" class="nav-link">AI</a>
        <a href="#" class="nav-link">소개</a>
      </nav>
      <div class="header-actions">
        <button class="btn-icon" onclick="toggleSearch()" title="검색" aria-label="검색">
    ...
    <div class="hero-inner fade-in">
      ...
      <div class="hero-stats">
        <div class="hero-stat">
          ...
          <span class="hero-stat-label">총 기사</span>
        </div>
        <div class="hero-stat">
          <span class="hero-stat-num">{num_dates}</span>
          <span class="hero-stat-label">일자</span>
        </div>
        <div class="hero-stat">
          <span class="hero-stat-num">{len([c for c in cat_counts if cat_counts[c] > 0])}</span>
...
{date_groups_html}
      ...
      <div class="ad-placeholder" style="margin-top: 16px;">
        ...
        <div style="font-size:11px; margin-top:4px; opacity:0.7;">기사 목록 하단에 자연스럽게 노출</div>
      </div>
    </main>
    ...
    <aside class="sidebar">
      ...
      <div class="ad-placeholder">
        ...
        <div style="font-size:11px; margin-top:4px; opacity:0.7;">사이드바 광고 영역</div>
      </div>
...
{archive_items}
      ...
      <div class="sidebar-widget">
        ...
        <p style="font-size:13px; color:var(--text-secondary); line-height:1.7;">
          ...
          카메라, 스마트폰, AI 분야의 중국 IT 뉴스를 가장 빠르게 만나보세요.
        </p>
      </div>
    </aside>
  </div>
